...
test.describe('A. Markdown Prevention (5 tests)', () => {
  ...
  test('A.1 Response contains no double asterisks (**)', async ({ page }) => {
    ...
    console.log('\n[TEST] TEST A.1: No Double Asterisks (**)');
    ...
    console.log('[OK] No double asterisks found');
  });

  test('A.2 Response contains no underscores (__)', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST A.2: No Underscores (__)');
    console.log('='.repeat(70));
    ...
    console.log('[OK] No markdown underscores found');
  });

  test('A.3 Response contains no backticks (`)', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST A.3: No Backticks (`)');
    console.log('='.repeat(70));
    ...
    console.log('[OK] No backticks found');
  });

  test('A.4 Response contains no pipes (|)', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST A.4: No Pipes (|)');
    console.log('='.repeat(70));
    ...
    console.log('[OK] No markdown pipes found');
  });

  test('A.5 Response contains no code block markers (``` or ~~~)', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST A.5: No Code Block Markers (``` or ~~~)');
    console.log('='.repeat(70));
    ...
    console.log('[OK] No code block markers found');
  });
...
test.describe('B. Prose Quality (5 tests)', () => {
  ...
  test('B.1 Response is continuous prose (no bullet points)', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST B.1: Continuous Prose (No Bullets)');
    console.log('='.repeat(70));
    ...
    console.log('[OK] No bullet points found');
  });

  test('B.2 Response has no numbered lists (1. 2. 3.)', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST B.2: No Numbered Lists');
    console.log('='.repeat(70));
    ...
    console.log('[OK] No numbered lists found');
  });

  test('B.3 Response has no headers (# ## ###)', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST B.3: No Headers');
    console.log('='.repeat(70));
  ...
  test('B.4 Response reads naturally (no "based on your data" formal phrases)', async ({ page }) => {
    ...
    console.log('[OK] No overly formal phrases detected');
  });

  test('B.5 Response is 1-2 paragraphs max', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST B.5: Paragraph Length (1-2 max)');
    console.log('='.repeat(70));
    ...
    console.log('[OK] Response is 1-2 paragraphs max');
  });
...
test.describe('C. Executive Coach Tone (4 tests)', () => {
  ...
  test('C.1 Warm and direct tone (detects "I notice" or similar)', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST C.1: Warm & Direct Tone');
    console.log('='.repeat(70));
    ...
    console.log('[OK] Warm and direct tone detected');
  });

  test('C.2 Specific with numbers (detects actual data)', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST C.2: Specific with Numbers');
    console.log('='.repeat(70));
    ...
    console.log('[OK] Response contains specific numbers');
  });

  test('C.3 Ends with actionable observation', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST C.3: Actionable Observation');
    console.log('='.repeat(70));
    ...
    console.log('[OK] Response ends with actionable observation');
  });

  test('C.4 No flattery, honest feedback', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST C.4: Honest Feedback (No Flattery)');
    console.log('='.repeat(70));
    ...
    console.log('[OK] Response avoids excessive flattery');
  });
...
test.describe('D. Streaming Behavior (4 tests)', () => {
  ...
  test('D.1 Stream produces valid SSE format', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST D.1: Valid SSE Format');
    console.log('='.repeat(70));
    ...
    if (streamResponse) {
      ...
      console.log('[OK] Stream has valid SSE content-type');
    } else {
      console.log('[WARN]  No stream response captured, but test continues');
    }

    // Verify response appears on page
    const messages = await page.locator('[class*="claude-message"]').count();
    expect(messages).toBeGreaterThan(0);
    console.log('[OK] SSE stream successfully delivered content to UI');
  });

  test('D.2 Stream accumulates text without duplication', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST D.2: Text Accumulation (No Duplication)');
    console.log('='.repeat(70));
    ...
    console.log(`[OK] No excessive word duplication (max repeat: ${maxRepeat}x)`);
  });

  test('D.3 Stream completes without hanging', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST D.3: Stream Completes Without Hanging');
    console.log('='.repeat(70));
    ...
    console.log('[OK] Stream completed successfully without hanging');
  });

  test('D.4 Stream handles 500+ character responses', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST D.4: Handles Long Responses (500+ chars)');
    console.log('='.repeat(70));
    ...
    console.log('[OK] Stream successfully delivered substantial response');
  });
...
test.describe('E. Edge Cases (2 tests)', () => {
  ...
  test('E.1 Responds correctly when user has no time entries', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST E.1: No Time Entries Edge Case');
    console.log('='.repeat(70));
    ...
    console.log('[OK] Gracefully handled no-entries scenario');
  });

  test('E.2 Handles API errors gracefully', async ({ page }) => {
    test.setTimeout(35000);
    console.log('\n[TEST] TEST E.2: API Error Handling');
    console.log('='.repeat(70));
    ...
    try {
      ...
      expect(response.length).toBeGreaterThan(0);
      console.log('[OK] Request handled successfully (or graceful error shown)');
    } catch (error) {
      ...
      console.log('[WARN]  Error occurred, but handled gracefully');
    }
  });
...
test.describe('Test Summary Report', () => {
  ...
  test('Generate comprehensive test report', async () => {
    console.log('\n' + '='.repeat(70));
    console.log('[data] COMPREHENSIVE TEST SUITE SUMMARY');
    console.log('='.repeat(70));

    console.log('\n[OK] A. MARKDOWN PREVENTION (5 tests)');
    ...
    console.log('\n[OK] C. EXECUTIVE COACH TONE (4 tests)');
    ...
    console.log('\n[OK] D. STREAMING BEHAVIOR (4 tests)');
