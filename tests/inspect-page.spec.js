...
test('Inspect page structure after login', async ({ page }) => {
  ...
  console.log(`\n[mic] Voice/Check-in visible: ${voiceCheckIn}`);
  ...
  console.log(`\n[input]  Type buttons found: ${typeButton}`);
  console.log(`[mic] Speak buttons found: ${speakButton}`);

  // Scroll to see all content
  const bodyHeight = await page.evaluate(() => document.body.scrollHeight);
  console.log(`\n[dim] Page height: ${bodyHeight}px`);

  // Get visible element IDs and classes
  const allElements = await page.locator('*[class*="check"], *[class*="chat"], *[class*="form"]').count();
  console.log(`[box] Elements with check/chat/form classes: ${allElements}`);

  // Print all text content
  const allText = await page.textContent('body');
  const lines = allText.split('\n').filter(l => l.trim().length > 0);
  console.log(`\n[doc] Page Text Content (first 30 lines):`);
  ...
  console.log(`\n[home] Dashboard visible: ${dashboardVisible}`);
});
